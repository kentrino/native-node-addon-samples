  ...
  for (unsigned int i = 0; i < length; i++) {
    if (!array->Get(i)->IsNumber()) {
      isolate->ThrowException(
          Exception::TypeError(String::NewFromUtf8(isolate, "Argument #3 must consist only of numbers.")));
    }
  ...
  for (unsigned int i = 0; i < length; ++i) {
    ...
    if (!local_key->IsString() || !local_val->IsString()) {
      isolate->ThrowException(Exception::TypeError(
          String::NewFromUtf8(isolate, "Argument #4 must have type of { [key: string]: string; }.")));
    }
...
void PrintEverything(const FunctionCallbackInfo<Value>& args) {
  ...
  if (!args[1]->IsString()) {
    isolate->ThrowException(Exception::TypeError(String::NewFromUtf8(isolate, "Argument #2 must be a string.")));
    return;
  }

  if (!args[2]->IsArray()) {
    isolate->ThrowException(Exception::TypeError(String::NewFromUtf8(isolate, "Argument #3 must be an array.")));
    return;
  }

  if (!args[3]->IsObject()) {
    isolate->ThrowException(Exception::TypeError(String::NewFromUtf8(isolate, "Argument #4 must be an object.")));
    return;
  }
